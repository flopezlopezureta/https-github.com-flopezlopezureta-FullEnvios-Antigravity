            ...
            >
                ...
                <div className="p-6 overflow-y-auto flex-1 bg-[var(--background-muted)]">
                    <div className="space-y-4">
                        {packages.map(pkg => (
                            <ShippingLabel key={pkg.id} pkg={pkg} creatorName={creatorName} />
                        ))}
                    </div>
                </div>
                ...
                <footer className="px-6 py-4 bg-[var(--background-muted)] rounded-b-xl flex justify-end space-x-3 border-t border-[var(--border-primary)]">
                    ...
                    <button type="button" onClick={handlePrint} className="px-4 py-2 text-sm font-medium text-white bg-[var(--brand-primary)] border border-transparent rounded-md shadow-sm hover:bg-[var(--brand-secondary)]">Imprimir Todo</button>
                </footer>
            </div>
        </div>
