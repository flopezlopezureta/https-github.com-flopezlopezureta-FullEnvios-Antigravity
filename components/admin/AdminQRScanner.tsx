...
const AdminQRScanner: React.FC = () => {
  ...
  const handleScan = useCallback(async (packageId: string) => {
    if(!isScanning) return; // Prevent multiple scans while one is processing
    setIsScanning(false);
    try {
      const result = await api.scanPackageByAdmin(packageId);
      ...
      setTimeout(() => {
        setScanResult(null);
        setIsScanning(true); // Resume scanning after error
      }, 4000);
    }
  }, [isScanning]);
  ...
  return (
    <div className="bg-[var(--background-secondary)] shadow-md rounded-lg p-6 max-w-2xl mx-auto">
      <h2 className="text-xl font-semibold text-[var(--text-primary)] mb-4 text-center">Escanear QR de Paquete</h2>
      ...
      <div className="h-16 mt-4">
        {scanResult ? (
            <div className={`flex items-center p-4 rounded-md text-white animate-fade-in-up ${scanResult.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`}>
             ...
             <p className="text-center text-[var(--text-muted)]">Apunta la cámara al código QR para actualizar el estado del paquete.</p>
        )}
      </div>
      <style>{`
